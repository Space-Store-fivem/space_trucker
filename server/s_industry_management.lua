-- gs_trucker/server/s_industry_management.lua (VERSÃO FINAL COM PREÇOS NO MONITOR)
...
CreateCallback('gs_trucker:callback:getIndustryStatus', function(source, cb)
    local SystemCompanyId = nil
    local result = MySQL.query.await('SELECT id FROM gs_trucker_companies WHERE name = ?', { 'Sistema' })
    if result and result[1] then SystemCompanyId = result[1].id end
    ...
    local allIndustries = Industries:GetIndustries()
    local purchasedIndustries = MySQL.query.await('SELECT * FROM gs_trucker_company_industries', {})
    local allStock = MySQL.query.await('SELECT * FROM gs_trucker_industry_stock', {})
