        ...
        for companyId, members in pairs(CompanyMemberData) do
            for src, _ in pairs(members) do
                if not GetPlayerEndpoint(src) then
                    print(('[space-trucker | GPS]: Jogador %s desconectado. Removendo dados de GPS.'):format(src))
                    CompanyMemberData[companyId][src] = nil
                    TriggerClientEvent('space_trucker:client:updateCompanyGPS', -1, CompanyMemberData[companyId])
                end
            end
        end
    end
...
RegisterNetEvent('space_trucker:server:updatePlayerGPS', function(data)
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
    if not player then return end

    local _, companyId = exports['space_trucker']:CheckIfPlayerWorksForCompany(src)
...
RegisterNetEvent('space_trucker:server:playerLeftCompany', function(companyId, targetSrc)
    local src = targetSrc or source
    if CompanyMemberData[companyId] and CompanyMemberData[companyId][src] then
        CompanyMemberData[companyId][src] = nil
        print(('[space-trucker | GPS]: Jogador %s saiu da empresa %s. Removendo dados.'):format(src, companyId))
        TriggerClientEvent('space_trucker:client:updateCompanyGPS', -1, CompanyMemberData[companyId])
    end
end)