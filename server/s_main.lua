...
-- Funções do Veículo
local function getVehicleModelMaxCapacity(_vehicleModel) return spaceconfig.VehicleTransport[_vehicleModel] and spaceconfig.VehicleTransport[_vehicleModel].capacity or 0 end
...
local function addVehicleItem(_vehicleModel, _vehiclePlate, _itemData, _amount)
    ...
    if not vehiclesCarryItemData[vehKey][_itemName] then
        ...
        vehiclesCarryItemData[vehKey][_itemName] = {
            ...
            transUnit = Lang:t(spaceconfig.ItemTransportUnit[_defaultItemData.transType]),
...
local function addVehicleProp(_vehicleModel, _vehiclePlate, _vehCoords, _itemTransType, _itemPropModel)
    if not (spaceconfig.AddVehicleProps and _itemPropModel and spaceconfig.VehicleTransport[_vehicleModel].props and spaceconfig.VehicleTransport[_vehicleModel].props[_itemTransType]) then return false end
...
local function checkVehicleCanAddItemToStorage(storageData, _industryTradeItemName)
    ...
    if storageData.currentCapacity > 0 then
        for itemName, _ in pairs(storageData.storage) do
            if not IsItemTransportTypeCanMix(spaceconfig.IndustryItems[itemName].transType) and itemName ~= _industryTradeItemName then
...
local function checkVehicleAndPlayerDistance(source, vehicleEntity)
    ...
    if #(playerCoords.xy - vehCoords.xy) > spaceconfig.MaxServerVehicleDistanceCheck then
    ...
    if _tradeState == spaceconfig.Industry.TradeState.onFoot then
        if playersCarryItemData[source] then
            ...
            if carryData.industryName == _industryName and carryData.itemName == _industryTradeItemName then
                if industryData:IsStorageFull(spaceconfig.Industry.TradeType.FORSALE, _industryTradeItemName) then return cb({ status = false, msg = Lang:t('storage_is_full') }) end
                if AddPlayerCash(source, math.floor(carryData.buyPrice * spaceconfig.ItemBackRate), ('back_item')) then
        ...
        if industryData:GetInStockAmount(spaceconfig.Industry.TradeType.FORSALE, _industryTradeItemName) <= 0 then return cb({ status = false, msg = Lang:t('industry_out_of_stock') }) end
        ...
        if industryData:GetInStockAmount(spaceconfig.Industry.TradeType.FORSALE, _industryTradeItemName) < buyAmount then return cb({ status = false, msg = Lang:t('industry_out_of_stock') }) end
        ...
        if addVehicleItem(vehModelHash, vehPlate, { buyPrice = itemTradeData.price, industryName = _industryName, itemName = _industryTradeItemName, addTime = os.time() }, buyAmount) then
            ...
            industryData:RemoveItemAmount(spaceconfig.Industry.TradeType.FORSALE, _industryTradeItemName, buyAmount, true)
...
CreateCallback('space_trucker:callback:sellItem', function(source, cb, _tradeState, _industryName, _industryTradeItemName, _extendArgs)
    ...
    if industryData:IsStorageFull(spaceconfig.Industry.TradeType.WANTED, _industryTradeItemName) then return cb({ status = false, msg = Lang:t('storage_is_full') }) end
...
CreateCallback('space_trucker:callback:loadPackageIntoVehicleByHand', function(source, cb, vehNetId)
    ...
    local itemProp = spaceconfig.IndustryItems[playerCarryItem.itemName].prop and spaceconfig.IndustryItems[playerCarryItem.itemName].prop.model or nil
