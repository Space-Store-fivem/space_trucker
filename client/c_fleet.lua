-- Space_trucker/client/c_fleet.lua (VERSÃO COM ORDEM CORRIGIDA)
...
local function checkJobAndGarage()
    print("^2[space-trucker DIAGNÓSTICO - CLIENTE]^7: A executar checkJobAndGarage...")
    QBCore.Functions.TriggerCallback('Space_trucker:callback:checkEmployment', function(isEmployed, companyId)
        print(string.format("^2[space-trucker DIAGNÓSTICO - CLIENTE]^7: Resposta do Servidor -> Empregado: ^3%s^7, ID da Empresa: ^3%s^7.", tostring(isEmployed), tostring(companyId)))
        
        PlayerCompanyId = isEmployed and companyId or nil
        
        if PlayerCompanyId then
            print("^2[space-trucker DIAGNÓSTICO - CLIENTE]^7: A pedir localização da garagem ao servidor...")
            QBCore.Functions.TriggerCallback('Space_trucker:callback:getGarageLocation', function(loc)
                if loc and loc.x then
                    print("^2[space-trucker DIAGNÓSTICO - CLIENTE]^7: ^2SUCESSO!^7 Coordenadas recebidas: ^3" .. json.encode(loc) .. "^7")
                    CurrentCompanyGarage = loc
                    updateBlip(PlayerCompanyId, loc)
                else
                    print("^1[space-trucker DIAGNÓSTICO - CLIENTE]^7: ^1FALHA!^7 O servidor respondeu, mas as coordenadas são inválidas ou nulas.")
...
CreateThread(function()
    ...
    while not hasPlayerLoaded do
        if QBCore and QBCore.Functions.GetPlayerData().citizenid then
            print("^2[space-trucker DIAGNÓSTICO - CLIENTE]^7: Dados do jogador carregados! A iniciar a verificação da garagem pela primeira vez.")
...
RegisterNetEvent('QBCore:Client:OnJobUpdate', function(job)
    print("^2[space-trucker DIAGNÓSTICO - CLIENTE]^7: O emprego do jogador foi atualizado. A verificar novamente a garagem...")
    checkJobAndGarage()
end)

-- COMANDO DE DEBUG MANUAL
RegisterCommand('checkgarage', function()
    print("^3[space-trucker DIAGNÓSTICO - CLIENTE]^7: O comando /checkgarage foi executado. A forçar a verificação.")
    checkJobAndGarage()
end, false)
...
function updateBlip(companyId, location)
    ...
    if location and companyId and companyId == PlayerCompanyId then
        print("^2[space-trucker DIAGNÓSTICO - CLIENTE]^7: A criar/atualizar o blip no mapa.")
