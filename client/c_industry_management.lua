...
RegisterNUICallback('getIndustryManagementData', function(data, cb)
    local result = TriggerCallbackAwait('gs_trucker:callback:getIndustryManagementData', { industryName = data.industryName })
    cb(result)
end)
...
RegisterNUICallback('investInIndustry', function(data, cb)
    TriggerServerEvent('gs_trucker:server:investInIndustry', data)
    cb('ok')
end)
...
RegisterNUICallback('hireNpcForIndustry', function(data, cb)
    TriggerServerEvent('gs_trucker:server:hireNpcForIndustry', data)
    cb('ok')
end)

-- Ouve o "aviso" do servidor de que uma atualização foi feita e reencaminha para a UI
RegisterNetEvent('gs_trucker:client:managementUpdate', function()
    SendNUIMessage({ action = 'managementUpdate' })
end)

-- Ouve as notificações do servidor e mostra-as ao jogador
RegisterNetEvent('gs_trucker:client:notify', function(message, type)
